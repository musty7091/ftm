...
class PosSettlementExcelFilterSelection:
    start_date: date
    end_date: date
    bank_id: int | None
    bank_account_id: int | None
    pos_device_id: int | None
    status: str
    currency_code: str
...
class PosSettlementExcelFilterDialog(QDialog):
    def __init__(
        self,
        ...
        *,
        ...
        self.setWindowTitle("POS Mutabakat Excel Filtreleri")
        self.setMinimumWidth(720)
        self.setModal(True)

        self.bank_options: list[_BankOption] = []
        self.bank_account_options: list[_BankAccountOption] = []
        self.pos_device_options: list[_PosDeviceOption] = []
        ...
        self.selected_filters: PosSettlementExcelFilterSelection | None = None

        self._load_options()
        self._build_ui()
        self._fill_bank_combo()
        self._fill_bank_account_combo(bank_id=None)
        self._fill_pos_device_combo(bank_id=None, bank_account_id=None)
        ...
        with session_scope() as session:
            ...
            pos_statement = (
                ...
                .order_by(
                    ...
                    BankAccount.account_name.asc(),
                    PosDevice.name.asc(),
                    PosDevice.terminal_no.asc(),
                    PosDevice.id.asc(),
                )
            )
            pos_rows = session.execute(pos_statement).all()

            for pos_device, bank_account, bank in pos_rows:
                self.pos_device_options.append(
                    _PosDeviceOption(
                        pos_device_id=int(pos_device.id),
                        bank_id=int(bank.id),
                        bank_account_id=int(bank_account.id),
                        bank_name=str(bank.name),
                        account_name=str(bank_account.account_name),
                        pos_device_name=str(pos_device.name),
                        terminal_no=pos_device.terminal_no,
                        is_active=bool(pos_device.is_active),
                    )
                )
    ...
    def _build_ui(self) -> None:
        ...
        title = QLabel("POS Mutabakat Excel Filtreleri")
        title.setObjectName("ReportSectionTitle")

        subtitle = QLabel(
            "Excel dosyasına aktarılacak POS mutabakat kayıtlarını tarih, banka, banka hesabı, POS cihazı, durum ve para birimine göre filtreleyebilirsin."
        )
        ...
        form_layout.addWidget(self._field_label("POS / Terminal"), 2, 0)
        form_layout.addWidget(self.pos_device_combo, 2, 1, 1, 3)

        form_layout.addWidget(self._field_label("Durum"), 3, 0)
        form_layout.addWidget(self.status_combo, 3, 1)

        form_layout.addWidget(self._field_label("Para Birimi"), 3, 2)
        form_layout.addWidget(self.currency_combo, 3, 3)

        main_layout.addWidget(panel)

        info = QLabel(
            "Not: Banka seçersen hesap ve POS listesi o bankaya göre daralır. "
            "Banka hesabı seçersen POS listesi sadece o hesaba bağlı cihazları gösterir."
        )
    ...
    def _fill_bank_account_combo(self, bank_id: int | None) -> None:
        current_account_id = self.bank_account_combo.currentData()
        ...
        if current_account_id is not None:
            index = self.bank_account_combo.findData(current_account_id)
            if index >= 0:
                self.bank_account_combo.setCurrentIndex(index)

        self.bank_account_combo.blockSignals(False)

    def _fill_pos_device_combo(
        self,
        *,
        bank_id: int | None,
        bank_account_id: int | None,
    ) -> None:
        current_pos_device_id = self.pos_device_combo.currentData()

        self.pos_device_combo.clear()
        self.pos_device_combo.addItem("Tüm POS Cihazları", None)

        for pos_option in self.pos_device_options:
            if bank_id is not None and pos_option.bank_id != bank_id:
                continue

            if bank_account_id is not None and pos_option.bank_account_id != bank_account_id:
                continue

            terminal_text = pos_option.terminal_no or "Terminal Yok"
            active_text = "Aktif" if pos_option.is_active else "Pasif"

            self.pos_device_combo.addItem(
                (
                    f"{pos_option.bank_name} / "
                    f"{pos_option.account_name} / "
                    f"{pos_option.pos_device_name} / "
                    f"{terminal_text} / "
                    f"{active_text}"
                ),
                pos_option.pos_device_id,
            )

        if current_pos_device_id is not None:
            index = self.pos_device_combo.findData(current_pos_device_id)
            if index >= 0:
                self.pos_device_combo.setCurrentIndex(index)

    def _on_bank_changed(self) -> None:
        selected_bank_id = self.bank_combo.currentData()
        bank_id = None if selected_bank_id is None else int(selected_bank_id)

        self._fill_bank_account_combo(bank_id=bank_id)
        self._fill_pos_device_combo(
            bank_id=bank_id,
            bank_account_id=self._selected_bank_account_id(),
        )

    def _on_bank_account_changed(self) -> None:
        self._fill_pos_device_combo(
    ...
    def _selected_bank_id(self) -> int | None:
        ...
        return None if selected_bank_id is None else int(selected_bank_id)

    def _selected_bank_account_id(self) -> int | None:
        selected_bank_account_id = self.bank_account_combo.currentData()

        return None if selected_bank_account_id is None else int(selected_bank_account_id)

    def _selected_pos_device_id(self) -> int | None:
        selected_pos_device_id = self.pos_device_combo.currentData()

        return None if selected_pos_device_id is None else int(selected_pos_device_id)
    ...
    def _accept_filters(self) -> None:
        ...
        self.selected_filters = PosSettlementExcelFilterSelection(
            start_date=start_date,
            end_date=end_date,
            bank_id=self._selected_bank_id(),
            bank_account_id=self._selected_bank_account_id(),
            pos_device_id=self._selected_pos_device_id(),
...
def get_pos_settlement_excel_filter_selection(
    parent: QWidget | None,
    *,
    default_start_date: date,
    default_end_date: date,
) -> PosSettlementExcelFilterSelection | None:
    dialog = PosSettlementExcelFilterDialog(
        parent,
...
__all__ = [
    "PosSettlementExcelFilterSelection",
    "PosSettlementExcelFilterDialog",
