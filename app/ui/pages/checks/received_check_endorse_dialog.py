...
@dataclass
class EndorsableReceivedCheckOption:
...
class ReceivedCheckEndorseDialog(QDialog):
    def __init__(self, *, parent: QWidget | None) -> None:
        super().__init__(parent)

        self.endorsable_checks = self._load_endorsable_checks()
        self.check_lookup = {
            endorsable_check.received_check_id: endorsable_check
            for endorsable_check in self.endorsable_checks
        }

        self.filtered_check_ids: list[int] = []
        self.payload: dict[str, Any] | None = None

        self.setWindowTitle("Alınan Çeki Kullan / Ciro Et")
        ...
        title = QLabel("Alınan Çeki Kullan / Ciro Et")
        title.setObjectName("SectionTitle")

        subtitle = QLabel(
            "Portföydeki alınan çeki seçer, kime verildiğini ve hangi amaçla kullanıldığını kaydedersin. "
            "Bu işlem banka hareketi oluşturmaz; çek portföyden çıkar ve ciro edilmiş olarak kapanır."
        )
        ...
        self.counterparty_input.setPlaceholderText("Çekin verildiği kişi / kurum / işletme")
        ...
        self.endorse_date_edit = QDateEdit()
        self.endorse_date_edit.setMinimumHeight(38)
        self.endorse_date_edit.setCalendarPopup(True)
        self.endorse_date_edit.setDisplayFormat("dd.MM.yyyy")
        self.endorse_date_edit.setDate(QDate.currentDate())
        form_layout.addRow("Kullanım tarihi", self.endorse_date_edit)
        ...
        self.save_button = QPushButton("Kullanımı Kaydet")
    ...
    def _load_endorsable_checks(self) -> list[EndorsableReceivedCheckOption]:
        with session_scope() as session:
            ...
            results: list[EndorsableReceivedCheckOption] = []
    ...
    def has_endorsable_checks(self) -> bool:
        return bool(self.endorsable_checks)

    def get_missing_data_message(self) -> str:
        return "Kullanılabilecek PORTFOLIO durumunda alınan çek kaydı bulunamadı."

    def _matches_search(self, endorsable_check: EndorsableReceivedCheckOption, search_text: str) -> bool:
        if not search_text:
            return True
        ...
        searchable_text = " | ".join(
            [
                str(endorsable_check.received_check_id),
                endorsable_check.customer_name,
                endorsable_check.check_number,
                endorsable_check.drawer_bank_name,
                endorsable_check.reference_no or "",
                endorsable_check.collection_bank_name or "",
                endorsable_check.collection_bank_account_name or "",
                endorsable_check.received_date.strftime("%d.%m.%Y"),
                endorsable_check.due_date.strftime("%d.%m.%Y"),
                _format_currency_amount(endorsable_check.amount, endorsable_check.currency_code),
            ]
        ).lower()

        return normalized_search_text in searchable_text

    def _matches_due_filter(self, endorsable_check: EndorsableReceivedCheckOption, filter_key: str) -> bool:
        today = date.today()
        due_date = endorsable_check.due_date
    ...
    def _apply_filters(self) -> None:
        ...
        filtered_checks = [
            endorsable_check
            for endorsable_check in self.endorsable_checks
            if self._matches_search(endorsable_check, search_text)
            and self._matches_due_filter(endorsable_check, filter_key)
        ]

        filtered_checks.sort(
            key=lambda endorsable_check: (
                endorsable_check.due_date,
                endorsable_check.customer_name.lower(),
                endorsable_check.check_number.lower(),
                endorsable_check.received_check_id,
            )
        )

        self.filtered_check_ids = [endorsable_check.received_check_id for endorsable_check in filtered_checks]
    ...
    def _fill_table(self, filtered_checks: list[EndorsableReceivedCheckOption]) -> None:
        self.checks_table.setRowCount(len(filtered_checks))

        today = date.today()

        for row_index, endorsable_check in enumerate(filtered_checks):
            values = [
                str(endorsable_check.received_check_id),
                endorsable_check.customer_name,
                endorsable_check.check_number,
                ...
                endorsable_check.due_date.strftime("%d.%m.%Y"),
                _format_currency_amount(endorsable_check.amount, endorsable_check.currency_code),
                _received_status_text(endorsable_check.status),
            ]

            is_overdue = endorsable_check.due_date < today
            is_due_soon = today <= endorsable_check.due_date <= today + timedelta(days=7)
            ...
            for column_index, value in enumerate(values):
                ...
                if column_index == 0:
                    item.setData(Qt.UserRole, endorsable_check.received_check_id)

                tooltip_lines = [
                    f"ID: {endorsable_check.received_check_id}",
                    f"Müşteri: {endorsable_check.customer_name}",
                    f"Çek No: {endorsable_check.check_number}",
                    f"Keşideci Banka: {endorsable_check.drawer_bank_name}",
                    f"Durum: {_received_status_text(endorsable_check.status)}",
                    f"Alınış: {endorsable_check.received_date.strftime('%d.%m.%Y')}",
                    f"Vade: {endorsable_check.due_date.strftime('%d.%m.%Y')}",
                    f"Tutar: {_format_currency_amount(endorsable_check.amount, endorsable_check.currency_code)}",
                ]

                if endorsable_check.collection_bank_name and endorsable_check.collection_bank_account_name:
                    tooltip_lines.append(
                        f"Mevcut Tahsil Hesabı: {endorsable_check.collection_bank_name} / {endorsable_check.collection_bank_account_name}"
                    )

                if endorsable_check.reference_no:
                    tooltip_lines.append(f"Referans No: {endorsable_check.reference_no}")
    ...
    def _update_results_info_label(self, filtered_count: int) -> None:
        total_count = len(self.endorsable_checks)

        if total_count == 0:
            self.results_info_label.setText("Kullanılabilecek PORTFOLIO durumunda çek bulunamadı.")
            return
    ...
    def _selected_check_from_table(self) -> EndorsableReceivedCheckOption | None:
    ...
    def _update_selected_check_info(self) -> None:
        ...
        if selected_check is None:
            self.info_label.setText("Kullanım / ciro için önce listeden bir çek seçmelisin.")
    ...
    def _build_payload(self) -> dict[str, Any]:
        ...
        if selected_check is None:
            raise ValueError("Kullanılacak / ciro edilecek çek seçilmelidir.")
        ...
        endorse_date = _qdate_to_date(self.endorse_date_edit.date())
