...
RETURN_REASON_OPTIONS = [
    ("NEW_CHECK_RECEIVED", "Yerine Yeni Çek Alındı"),
    ("CASH_PAYMENT_RECEIVED", "Yerine Nakit Ödeme Alındı"),
    ("BANK_TRANSFER_RECEIVED", "Yerine Banka / Havale Ödemesi Alındı"),
    ("AGREEMENT_CANCELLED", "Anlaşma / Sipariş İptal Edildi"),
    ("INVOICE_OR_PROCESS_CANCELLED", "Fatura / İşlem İptal Edildi"),
    ("WRONG_CHECK_RECEIVED", "Hatalı Çek Alındı"),
    ("DUPLICATE_OR_WRONG_RECORD", "Mükerrer Kayıt / Yanlış Kayıt"),
    ("CUSTOMER_REQUEST", "Müşteri Talebiyle İade Edildi"),
    ("OTHER", "Diğer"),
]


RETURN_REASON_DESCRIPTION_REQUIRED_CODES = {
    "OTHER",
}


class ReceivedCheckReturnDialog(QDialog):
    def __init__(self, *, parent: QWidget | None) -> None:
        ...
        title = QLabel("Alınan Çeki İade İşaretle")
        ...
        form_layout.addRow("İade tarihi", self.return_date_edit)

        self.counterparty_input = QLineEdit()
        self.counterparty_input.setMinimumHeight(38)
        ...
        self.description_input.setPlaceholderText(
            "İsteğe bağlı açıklama yazınız. 'Diğer' nedeni seçilirse açıklama zorunludur."
        )
        ...
        self.save_button = QPushButton("İade İşaretle")
    ...
    def _load_returnable_checks(self) -> list[ReturnableReceivedCheckOption]:
        with session_scope() as session:
            ...
            results: list[ReturnableReceivedCheckOption] = []
    ...
    def get_missing_data_message(self) -> str:
        return "İade işaretlenebilecek alınan çek kaydı bulunamadı."

    def _matches_search(self, returnable_check: ReturnableReceivedCheckOption, search_text: str) -> bool:
        if not search_text:
            return True
        ...
        collection_text = (
            f"{returnable_check.collection_bank_name} / {returnable_check.collection_bank_account_name}"
            if returnable_check.collection_bank_name and returnable_check.collection_bank_account_name
            else ""
        )

        searchable_text = " | ".join(
            [
                str(returnable_check.received_check_id),
                returnable_check.customer_name,
                returnable_check.drawer_bank_name,
                collection_text,
                returnable_check.check_number,
                returnable_check.received_date.strftime("%d.%m.%Y"),
                returnable_check.due_date.strftime("%d.%m.%Y"),
                format_currency_amount(returnable_check.amount, returnable_check.currency_code),
                received_status_text(returnable_check.status),
                returnable_check.reference_no or "",
                returnable_check.description or "",
            ]
        ).lower()
    ...
    def _apply_filters(self) -> None:
        ...
        filtered_checks = [
            returnable_check
            for returnable_check in self.returnable_checks
            if self._matches_search(returnable_check, search_text)
        ]

        filtered_checks.sort(
            key=lambda returnable_check: (
                returnable_check.due_date,
                returnable_check.customer_name.lower(),
                returnable_check.check_number.lower(),
                returnable_check.received_check_id,
            )
        )
    ...
    def _fill_table(self, filtered_checks: list[ReturnableReceivedCheckOption]) -> None:
        self.checks_table.setRowCount(len(filtered_checks))

        today = date.today()

        for row_index, returnable_check in enumerate(filtered_checks):
            collection_text = (
                f"{returnable_check.collection_bank_name} / {returnable_check.collection_bank_account_name}"
                if returnable_check.collection_bank_name and returnable_check.collection_bank_account_name
                else "-"
            ...
            is_overdue = returnable_check.due_date < today
            is_due_soon = today <= returnable_check.due_date <= today + timedelta(days=7)

            for column_index, value in enumerate(values):
                item = QTableWidgetItem(value)

                if column_index == 0:
                    item.setData(Qt.UserRole, returnable_check.received_check_id)

                if returnable_check.status == "BOUNCED":
                ...
                tooltip_lines = [
                    f"ID: {returnable_check.received_check_id}",
                    f"Müşteri: {returnable_check.customer_name}",
                    f"Keşideci Banka: {returnable_check.drawer_bank_name}",
                    f"Tahsil Hesabı: {collection_text}",
                    f"Çek No: {returnable_check.check_number}",
                    f"Durum: {received_status_text(returnable_check.status)}",
                    f"Alınış: {returnable_check.received_date.strftime('%d.%m.%Y')}",
                    f"Vade: {returnable_check.due_date.strftime('%d.%m.%Y')}",
                    f"Tutar: {format_currency_amount(returnable_check.amount, returnable_check.currency_code)}",
                ]

                if returnable_check.reference_no:
                    tooltip_lines.append(f"Referans No: {returnable_check.reference_no}")

                if returnable_check.description:
                    tooltip_lines.append(f"Açıklama: {returnable_check.description}")
    ...
    def _update_results_info_label(self, filtered_count: int) -> None:
        total_count = len(self.returnable_checks)

        if total_count == 0:
            self.results_info_label.setText("İade işaretlenebilecek alınan çek kaydı bulunamadı.")
            return

        if filtered_count == 0:
            self.results_info_label.setText("Filtreye uygun kayıt bulunamadı. Arama metnini değiştir.")
            return

        self.results_info_label.setText(
            f"Toplam {total_count} iade işaretlenebilir alınan çek içinden {filtered_count} kayıt listeleniyor. "
    ...
    def _selected_check_from_table(self) -> ReturnableReceivedCheckOption | None:
    ...
    def _update_selected_check_info(self) -> None:
        ...
        if selected_check.customer_name and not self.counterparty_input.text().strip():
            self.counterparty_input.setText(selected_check.customer_name)
    ...
    def _build_payload(self) -> dict[str, Any]:
        ...
        if selected_check is None:
            raise ValueError("İade işaretlenecek alınan çek seçilmelidir.")

        return_date = _qdate_to_date(self.return_date_edit.date())
        ...
        if not counterparty_text:
            raise ValueError("İade edilen taraf boş olamaz.")

        if len(counterparty_text) < 2:
            raise ValueError("İade edilen taraf en az 2 karakter olmalıdır.")

        if not return_reason_code:
            raise ValueError("İade nedeni seçilmelidir.")

        if return_reason_code in RETURN_REASON_DESCRIPTION_REQUIRED_CODES and not description_text:
            raise ValueError("Diğer nedeni seçildiğinde açıklama yazılmalıdır.")
