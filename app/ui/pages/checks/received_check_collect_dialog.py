...
@dataclass
class CollectableReceivedCheckOption:
...
class ReceivedCheckCollectDialog(QDialog):
    def __init__(self, *, parent: QWidget | None) -> None:
        super().__init__(parent)

        self.collectable_checks = self._load_collectable_checks()
        self.collection_bank_accounts = self._load_collection_bank_accounts()

        self.check_lookup = {
            collectable_check.received_check_id: collectable_check
            for collectable_check in self.collectable_checks
        }
        ...
        self.setWindowTitle("Alınan Çek Tahsil Et")
        ...
        title = QLabel("Alınan Çek Tahsil Et")
        title.setObjectName("SectionTitle")

        subtitle = QLabel(
            "Tahsil edilebilir durumdaki alınan çeki seçer, uygun banka hesabına tahsil işlemini tamamlarsın. "
            "Arama ve vade filtresiyle kayıtlar daha hızlı bulunur."
        )
        ...
        self.collection_date_edit.setMinimumHeight(38)
        ...
        form_layout.addRow("Tahsil tarihi", self.collection_date_edit)
        ...
        self.save_button = QPushButton("Tahsilatı Kaydet")
    ...
    def _load_collectable_checks(self) -> list[CollectableReceivedCheckOption]:
        with session_scope() as session:
            ...
            results: list[CollectableReceivedCheckOption] = []
            ...
            for received_check, customer, collection_bank_account, collection_bank in rows:
                ...
                results.append(
                    CollectableReceivedCheckOption(
    ...
    def has_collectable_checks(self) -> bool:
        return bool(self.collectable_checks)

    def get_missing_data_message(self) -> str:
        return "Tahsil edilebilir durumdaki alınan çek kaydı bulunamadı."

    def _matches_search(self, collectable_check: CollectableReceivedCheckOption, search_text: str) -> bool:
        if not search_text:
            return True
        ...
        searchable_text = " | ".join(
            [
                str(collectable_check.received_check_id),
                collectable_check.customer_name,
                collectable_check.check_number,
                collectable_check.drawer_bank_name,
                collectable_check.reference_no or "",
                collectable_check.collection_bank_name or "",
                collectable_check.collection_bank_account_name or "",
                collectable_check.received_date.strftime("%d.%m.%Y"),
                collectable_check.due_date.strftime("%d.%m.%Y"),
                _format_currency_amount(collectable_check.amount, collectable_check.currency_code),
            ]
        ).lower()

        return normalized_search_text in searchable_text

    def _matches_due_filter(self, collectable_check: CollectableReceivedCheckOption, filter_key: str) -> bool:
        today = date.today()
        due_date = collectable_check.due_date
    ...
    def _apply_filters(self) -> None:
        ...
        filtered_checks = [
            collectable_check
            for collectable_check in self.collectable_checks
            if self._matches_search(collectable_check, search_text)
            and self._matches_due_filter(collectable_check, filter_key)
        ]

        filtered_checks.sort(
            key=lambda collectable_check: (
                collectable_check.due_date,
                collectable_check.customer_name.lower(),
                collectable_check.check_number.lower(),
                collectable_check.received_check_id,
            )
        )

        self.filtered_check_ids = [collectable_check.received_check_id for collectable_check in filtered_checks]
    ...
    def _fill_table(self, filtered_checks: list[CollectableReceivedCheckOption]) -> None:
        self.checks_table.setRowCount(len(filtered_checks))

        today = date.today()

        for row_index, collectable_check in enumerate(filtered_checks):
            values = [
                str(collectable_check.received_check_id),
                collectable_check.customer_name,
                collectable_check.check_number,
                ...
                collectable_check.due_date.strftime("%d.%m.%Y"),
                _format_currency_amount(collectable_check.amount, collectable_check.currency_code),
                _received_status_text(collectable_check.status),
            ]

            is_overdue = collectable_check.due_date < today
            is_due_soon = today <= collectable_check.due_date <= today + timedelta(days=7)
            ...
            for column_index, value in enumerate(values):
                ...
                if column_index == 0:
                    item.setData(Qt.UserRole, collectable_check.received_check_id)

                tooltip_lines = [
                    f"ID: {collectable_check.received_check_id}",
                    f"Müşteri: {collectable_check.customer_name}",
                    f"Çek No: {collectable_check.check_number}",
                    f"Keşideci Banka: {collectable_check.drawer_bank_name}",
                    f"Durum: {_received_status_text(collectable_check.status)}",
                    f"Alınış: {collectable_check.received_date.strftime('%d.%m.%Y')}",
                    f"Vade: {collectable_check.due_date.strftime('%d.%m.%Y')}",
                    f"Tutar: {_format_currency_amount(collectable_check.amount, collectable_check.currency_code)}",
                ]

                if collectable_check.collection_bank_name and collectable_check.collection_bank_account_name:
                    tooltip_lines.append(
                        f"Tahsil Hesabı: {collectable_check.collection_bank_name} / {collectable_check.collection_bank_account_name}"
                    )

                if collectable_check.reference_no:
                    tooltip_lines.append(f"Referans No: {collectable_check.reference_no}")
    ...
    def _update_results_info_label(self, filtered_count: int) -> None:
        total_count = len(self.collectable_checks)

        if total_count == 0:
            self.results_info_label.setText("Tahsil edilebilir durumdaki alınan çek kaydı bulunamadı.")
            return
        ...
        self.results_info_label.setText(
            f"Toplam {total_count} açık alınan çek içinden {filtered_count} kayıt listeleniyor. "
    ...
    def _selected_check_from_table(self) -> CollectableReceivedCheckOption | None:
    ...
    def _fill_collection_account_combo_for_selected_check(self, selected_check: CollectableReceivedCheckOption | None) -> None:
    ...
    def _update_collection_account_info(self) -> None:
        ...
        if selected_collection_account_id is None:
            self.collection_account_info_label.setText(
                ...
                "Tahsil işlemi için aynı para biriminde aktif banka hesabı seçilmelidir."
            )
            return
    ...
    def _update_selected_check_info(self) -> None:
        ...
        if selected_check is None:
            self.info_label.setText("Tahsilat için önce listeden bir çek seçmelisin.")
    ...
    def _build_payload(self) -> dict[str, Any]:
        ...
        if selected_check is None:
            raise ValueError("Tahsil edilecek çek seçilmelidir.")
